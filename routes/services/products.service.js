...
const { imageDelete, deleteImagesWithProduct } = require('../../utils/imageDelete.util')
...
const getProductsByQuantity = (req, res, next) => {
  ...
  return Product.getProductsByQuantity(limit, offset)
    .then((data) => {
      const {rows: products, totalItems} = getPagingData(data, page)
      const totalPages = Math.ceil(totalItems/limit)
      return res.render('announcements', { products, totalPages })
    })
    .catch(next)
}
...
const deleteProductById = (req, res, next) => {
  ...
  return Product.deleteProduct(id)
    .then(() => deleteImagesWithProduct(id))
    .catch(next)
}
